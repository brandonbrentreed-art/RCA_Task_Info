...
var NdpDemand = (function () {
  ...
  function buildEnrichTable() {
    ...
    var enrichHeaders = NdpData.state.enrichHeaders;
    var slaIdx = NdpData.state.enrichSlaIdx;
    var dwellIdx = NdpData.state.enrichDwellIdx;
    var idIdx = NdpData.state.enrichIdIdx;

    // Build ID → OUC lookup from taskforce (try task ID and service ID columns)
    ...
    // Also find service ID in taskforce if available
    var tfServiceIdx = -1;
    ...
    // Collect all unique age buckets across all OUCs for column headers
    var allBuckets = {};
    ...
    var bucketCols = Object.keys(allBuckets).sort(function (a, b) {
      // Extract number from RT-XX and sort highest first
